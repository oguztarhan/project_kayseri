...
namespace Game.UI
{
    ...
    public sealed class HudDebug : MonoBehaviour
    {
        ...
        {
            if (_mountains == null || _cam == null || _mountains.Mountains == null) return;
            float w = Screen.width, h = Screen.height;
            ...
            for (int i = 0; i < ms.Length; i++)
            {
                ...
                Vector3 sp = _cam.WorldToScreenPoint(m.LabelAnchor);
                if (sp.z <= 0f) continue;                 // behind the camera
                float gx = sp.x, gy = h - sp.y;           // GUI space is top-down
                float bw = 210f * s, bh = 52f * s;
                float rx = Mathf.Clamp(gx - bw / 2f, 4f, w - bw - 4f);
                float ry = Mathf.Clamp(gy - bh, topH + 4f, botLimit - bh - 4f);
                BigDouble c = new BigDouble(_mountains.Cost(m));
                GUI.enabled = _wallet.Cash >= c;
                if (GUI.Button(new Rect(rx, ry, bw, bh), "Unlock " + m.Def.DisplayName + "\n$" + NumberFormatter.Format(c), _worldBtn))
                    _mountains.TryUnlock(m);
                GUI.enabled = true;
            }
        }
