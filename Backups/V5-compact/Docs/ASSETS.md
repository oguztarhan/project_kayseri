...
## M. Assets I need (running list — added during the V3 real-GDD build)

*(These would upgrade greybox/reused meshes that are standing in now. Everything is a reference swap — zero code change. Prepend the §0 Global Style Guide to each Prompt.)*

- **M1 · Locked-Mountain Padlock Marker** — shown floating over a mountain until the player unlocks it. **Prompt:** *"a chunky low-poly padlock icon on a short post/plinth, bright gold shackle + dark body, readable from the iso camera; sits above a mountain base; ~1.5 u tall."* (<150 tris, pivot base-centre.) *Used by `MountainMine.lockedMarker`.*
