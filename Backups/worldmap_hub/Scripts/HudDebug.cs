...
namespace Game.UI
{
    ...
    public sealed class HudDebug : MonoBehaviour
    {
        ...
        [SerializeField] private bool _showIslandLabels; // island unlock/upgrade now lives on the WorldMapUI hub; off by default
        ...
        private bool _init;
        private GUIStyle _bar, _panel, _cur, _curR, _rate, _btn, _tabOn, _tabOff, _title, _worldBtn, _worldLbl, _worldLockLbl;
        ...
        private void EnsureStyles(float s)
        {
            if (!_init)
            {
                ...
                _worldLbl = new GUIStyle(_worldBtn) { fontStyle = FontStyle.Bold };
                _worldLbl.normal.background = Solid(new Color(0.10f, 0.30f, 0.20f, 0.75f)); _worldLbl.normal.textColor = new Color(0.55f, 0.95f, 0.6f);
                _worldLockLbl = new GUIStyle(_worldBtn) { fontStyle = FontStyle.Bold };
                _worldLockLbl.normal.background = Solid(new Color(0.16f, 0.18f, 0.22f, 0.72f)); _worldLockLbl.normal.textColor = new Color(0.7f, 0.72f, 0.78f);
                _init = true;
            }
            _cur.fontSize = _curR.fontSize = (int)(30 * s);
            _rate.fontSize = (int)(22 * s);
            _worldBtn.fontSize = _worldLbl.fontSize = _worldLockLbl.fontSize = (int)(17 * s);
        ...
        private void DrawWorldUnlocks(float s, float topH)
        {
            ...
            // floating island labels over the archipelago: upgrade the current island, max it, then the
            // NEXT island becomes unlockable — locked islands further out just show a padlock hint.
            // (Disabled by default: the WorldMapUI hub now owns island unlock/upgrade.)
            if (_showIslandLabels && _islands != null && _islands.Islands != null)
            {
                var isls = _islands.Islands;
                for (int i = 0; i < isls.Length; i++)
                {
                    Island isl = isls[i];
                    if (isl == null || isl.Def == null) continue;
                    if (!ToRect(isl.LabelAnchor, bw, bh, topH, botLimit, out Rect r)) continue;

                    if (isl.Unlocked)
                    {
                        if (isl.IsMaxed)
                            GUI.Label(r, isl.Def.DisplayName + "  MAX\n+" + NumberFormatter.Format(new BigDouble(isl.IncomePerSec)) + "/s", _worldLbl);
                        else
                        {
                            BigDouble uc = new BigDouble(isl.UpgradeCost);
                            GUI.enabled = _wallet.Cash >= uc;
                            if (GUI.Button(r, "Upgrade " + isl.Def.DisplayName + " Lv." + isl.Level + "/" + isl.MaxLevel + "\n$" + NumberFormatter.Format(uc), _worldBtn)) _islands.TryUpgrade(isl);
                            GUI.enabled = true;
                        }
                    }
                    else if (_islands.CanUnlock(isl))
                    {
                        BigDouble c = new BigDouble(_islands.Cost(isl));
                        GUI.enabled = _wallet.Cash >= c;
                        if (GUI.Button(r, "Unlock " + isl.Def.DisplayName + "\n$" + NumberFormatter.Format(c), _worldBtn)) _islands.TryUnlock(isl);
                        GUI.enabled = true;
                    }
                    else
                        GUI.Label(r, "[Locked]\n" + isl.Def.DisplayName, _worldLockLbl);
                }
            }
        }
