...
**Remaining M1:** HUD (currency bar via NumberFormatter, per-station upgrade buttons using IUpgradable, pooled floating text); persist station levels in SaveData (wallet already persists); final human-playable PlayMode pass. Then snapshot `Backups/M1/`.
