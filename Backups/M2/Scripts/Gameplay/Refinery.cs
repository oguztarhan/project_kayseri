...
{
    /// <summary>
    /// Turns raw ore into products via recipes each tick (GDD §4). Input inventory is filled by ore
    /// trucks, output inventory is drained by cargo trucks. Handles 1:1 (Coke) and combine (Steel).
    /// Recipe input arrays are cached once to keep the tick allocation-free (GDD §14.5).
    /// </summary>
    public sealed class Refinery : MonoBehaviour, IUpgradable
    {
        ...
        private void OnTick()
        {
            ...
            BigDouble batchBudget = new BigDouble(config.RateAtLevel(Level) * _clock.TickInterval);
