...
namespace Game.UI
{
    ...
    public sealed class WorldMapUI : MonoBehaviour
    {
        ...
        [SerializeField] private Vector3 mapLook = new Vector3(0f, 0f, 60f);
        [SerializeField] private Vector3 mapRotEuler = new Vector3(58f, 35f, 0f);
        [SerializeField] private float mapBack = 160f;
        [SerializeField] private float mapSize = 98f;
        [SerializeField] private Vector2 mapZoom = new Vector2(45f, 135f);
        [SerializeField] private Vector2 mapPan = new Vector2(95f, 130f);   // half-extents around the framing

        [Header("Island camera profile")]
        [SerializeField] private Vector3 islandRotEuler = new Vector3(50f, 45f, 0f);
        [SerializeField] private float islandBack = 72f;
        [SerializeField] private float islandSize = 30f;
        [SerializeField] private Vector2 islandZoom = new Vector2(8f, 46f);
        [SerializeField] private float islandPan = 48f;
        ...
        private void Start()
        {
            ...
            if (_cam != null)
            {
                _homePos = _cam.transform.position; _homeRot = _cam.transform.rotation;
                _homeSize = _cam.orthographic ? _cam.orthographicSize : 26f;
                _cam.farClipPlane = Mathf.Max(_cam.farClipPlane, 3000f);   // so pulling back to map framing never clips
            }
        ...
        private void EnterIsland(Island isl)
        {
            ...
            Quaternion rot; Vector3 pos; float size;
            if (isl.Def != null && isl.Def.HomeIsland) { pos = _homePos; rot = _homeRot; size = _homeSize; }
            else { rot = Quaternion.Euler(islandRotEuler); pos = isl.transform.position + rot * Vector3.forward * -islandBack; size = islandSize; }
