...
{
    /// <summary>
    /// Sells finished products out of its inventory each tick and pays the player (GDD §3, §4). A
    /// manager multiplies its sell rate (§6); the prestige multiplier boosts revenue (§8).
    /// </summary>
    public sealed class Market : MonoBehaviour, IUpgradable, IProducer
    {
        ...
        private void OnTick()
        {
            ...
            if (_prestige != null) revenue = revenue * _prestige.IncomeMultiplier;
