...
### V4 status
Done + verified: **camera hero-framing + toony lighting**, **ghosted tappable mountain unlocks**, **vehicle headway/queueing**. Already-done from prior turns: real meshes everywhere, rail/road network, tiled ground + dressing, chain-follow wagons + turn-to-face movement, touch pan/pinch + tap-to-mine.
**Still open:** the **parking-lot depot with ghost truck-bays** that fill on fleet purchase (world-labels for +truck not yet placed), a **toon outline** shader, and replacing the touch-scaled **IMGUI HudDebug with a real uGUI canvas** (§12 canvas-split) — the biggest remaining item.
