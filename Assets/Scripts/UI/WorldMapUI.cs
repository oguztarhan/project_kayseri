...
namespace Game.UI
{
    ...
    public sealed class WorldMapUI : MonoBehaviour
    {
        ...
        [Header("Operation view (boot straight into the working chain)")]
        [SerializeField] private string operationRootName = "Island_Coal";
        [SerializeField] private Vector3 opRotEuler = new Vector3(52f, 45f, 0f);
        [SerializeField] private float opBack = 170f;
        [SerializeField] private float opViewPad = 0.62f;   // ortho size = operation span * this
        [SerializeField] private float opPan = 130f;        // pan half-extent around the operation

        private WalletService _wallet;
        private IslandManager _islands;
        private Camera _cam;
        private CameraController _camCtrl;
        private HudUGUI _hud;
        ...
        private void Start()
        {
            ...
            if (_cam != null) _cam.farClipPlane = Mathf.Max(_cam.farClipPlane, 20000f);   // pulling back to map framing must not clip

            _circle = MakeCircle(); _flat = MakeFlat();
            Build();
            EnterOperation();   // boot straight into the real working chain, not the (secondary) world map
        }

        private void Update()
        {
            if (_wallet == null) _wallet = ServiceLocator.Get<WalletService>();   // resolve late if services came up after us; never freeze the map/enter on it
            EnforceHud();   // keep the HUD hidden on the map / shown in an island, regardless of Start() order
            HandlePointer();
        ...
        {
            if (isl.Unlocked)
                return isl.IsMaxed ? "<color=#" + GoldHex + ">★ MAX</color>"
                                   : "<color=#" + GreenHex + ">Lv " + isl.TotalLevel + "</color>";
        ...
        private void UpdateCard(Island isl)
        {
            ...
            if (unlocked)
            {
                ...
                _cardInfo.text = inc + "     enter to upgrade";
            }
            else _cardInfo.text = can ? "Ready to claim — unlock to start mining" : "Locked — max the previous island first";

            // Upgrades now live inside the island's own tab. The card's left button = Unlock/Locked/Maxed status;
            // the right button = Enter (shown only when unlocked).
            _cardAct.onClick.RemoveAllListeners();
            if (unlocked) { _cardActTxt.text = maxed ? "★ MAXED" : "OPEN ISLAND"; _cardActBg.color = maxed ? Gold : Green; _cardAct.interactable = false; }
            else if (can)
            {
        ...
        {
            if (_hud == null) _hud = FindAnyObjectByType<HudUGUI>();
            if (_hudCanvas == null && _hud != null) _hudCanvas = _hud.GetComponentInChildren<Canvas>();
            if (_hudDebug == null) _hudDebug = FindAnyObjectByType<HudDebug>();
        }

        // Keep the game HUD in sync with the mode every frame — the HUD builds its canvas in its own Start(),
        // so if it comes up after our OpenMap() ran, it would otherwise stay visible on the map.
        private void EnforceHud()
        {
            ResolveHud();
            bool showHud = _mode == Mode.Island;
            if (_hudCanvas != null && _hudCanvas.enabled != showHud) _hudCanvas.enabled = showHud;
        }
        ...
        private void OpenMap()
        {
            ...
            if (_islands != null) _islands.ShowAll();      // all islands visible on the world map
            if (_hud != null) _hud.SetCurrentIsland(null); // no island's upgrade tab on the map
            RefreshStates();
        }
        ...
        private void EnterIsland(Island isl)
        {
            ...
            if (_islands != null) _islands.ShowOnly(isl);   // hide every other island while inside this one
            if (_hud != null) _hud.SetCurrentIsland(isl);   // the HUD's upgrade tab now shows THIS island's tracks
        }

        /// <summary>Boot straight into the working production chain — the real mine → train → storage → truck →
        /// smelter → truck → market operation — framed for free pan/zoom. This is the game the player actually
        /// plays; the world map stays available behind it. Frames on the operation's true bounds (ignoring the
        /// big water/ground planes) and points the HUD's upgrade tab at the real stations via the home island.</summary>
        private void EnterOperation()
        {
            ...
            Vector3 c = Vector3.zero; float span = 100f;
            ...
            if (root != null)
            {
                ...
                {
                    if (child.name == "Water" || child.name == "Ground" || child.name == "GroundTiles" ||
                        child.name == "isle_Coal" || child.name == "lagoon_Coal") continue;   // skip the big ground/water planes
                ...
                if (have) { c = b.center; span = Mathf.Max(b.size.x, b.size.z); }
            }

            Quaternion rot = Quaternion.Euler(opRotEuler);
            float size = Mathf.Max(30f, span * opViewPad);
            Vector3 pos = c + rot * Vector3.forward * -opBack;
            if (_camCtrl != null)
            {
                _camCtrl.enabled = true;
                _camCtrl.SetZoomRange(Mathf.Max(12f, size * 0.35f), size * 1.9f);
                _camCtrl.SetBounds(new Vector2(pos.x - opPan, pos.x + opPan), new Vector2(pos.z - opPan, pos.z + opPan));
                _camCtrl.FrameTo(pos, rot, size);
            }
            else if (_cam != null) { _cam.transform.SetPositionAndRotation(pos, rot); if (_cam.orthographic) _cam.orthographicSize = size; }

            // The real chain's upgrades live on the home island's tab (it routes to GameWorld.Groups).
            Island home = null;
            if (_islands != null && _islands.Islands != null)
                for (int i = 0; i < _islands.Islands.Length; i++)
                {
                    Island isl = _islands.Islands[i];
                    if (isl != null && isl.Def != null && isl.Def.HomeIsland) { home = isl; break; }
                }
            if (home == null && _islands != null && _islands.Islands != null && _islands.Islands.Length > 0) home = _islands.Islands[0];
            if (_hud != null) _hud.SetCurrentIsland(home);
        }
