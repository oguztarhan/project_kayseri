using Game.Core;
using Game.Systems;
using UnityEngine;

namespace Game.Gameplay
{
    /// <summary>
    /// Drives the archipelago progression (GDD §4/§8). The player starts on the Coal home island, unlocks the
    /// first ore island with cash, then <b>maxes each island's upgrades before the next one can be unlocked</b> —
    /// "goes on like that". Every unlocked island keeps earning cash forever. The HUD reads this to show the
    /// floating "Unlock / Upgrade &lt;Ore&gt; Island" labels over each island.
    /// </summary>
    public sealed class IslandManager : MonoBehaviour
    {
        [SerializeField] private Island[] islands;   // in unlock order (Iron → Copper → … → Diamond)

        private WalletService _wallet;
        private SaveData _data;

        public Island[] Islands => islands;

        private void Start()
        {
            _wallet = ServiceLocator.Get<WalletService>();
            _data = ServiceLocator.Get<SaveData>();
            if (islands == null || _data == null) return;
            for (int i = 0; i < islands.Length; i++)
            {
                Island isl = islands[i];
                if (isl == null || isl.Def == null) continue;
                if (_data.unlockedIslands.Contains(isl.Def.name)) isl.Unlock();
                string name = isl.Def.name;
                for (int j = 0; j < _data.islandLevels.Count; j++)
                {
                    string id = _data.islandLevels[j].id;
                    if (id == null || !id.StartsWith(name)) continue;
                    if (id.Length > name.Length && id[name.Length] == '#')          // new per-track key "<name>#<t>"
                    {
                        int track;
                        if (int.TryParse(id.Substring(name.Length + 1), out track)) isl.SetTrackLevel(track, _data.islandLevels[j].level);
                    }
                    else if (id == name) isl.SetTrackLevel(0, _data.islandLevels[j].level);   // legacy single-level save → seed track 0
                }
            }
        }

        public bool IsUnlocked(Island i) => i != null && i.Unlocked;
        public double Cost(Island i) => (i != null && i.Def != null) ? i.Def.UnlockCost : 0d;

        /// <summary>Index of an island in the ordered array, or -1.</summary>
        public int IndexOf(Island i)
        {
            if (islands == null || i == null) return -1;
            for (int k = 0; k < islands.Length; k++) if (islands[k] == i) return k;
            return -1;
        }

        /// <summary>An island can be bought only once the previous island in the chain is fully maxed.</summary>
        public bool CanUnlock(Island i)
        {
            if (i == null || i.Unlocked) return false;
            int idx = IndexOf(i);
            if (idx <= 0) return idx == 0;
            Island prev = islands[idx - 1];
            if (prev == null) return false;
            // the Coal home island runs the real chain (its Island tracks never "max"), so it doesn't gate;
            // every other island must have all its tracks maxed before the next unlocks.
            return prev.IsMaxed || (prev.Def != null && prev.Def.HomeIsland);
        }

        /// <summary>The single locked island the player is currently working toward (first locked in the chain).</summary>
        public Island NextUnlockable
        {
            get
            {
                if (islands == null) return null;
                for (int k = 0; k < islands.Length; k++)
                    if (islands[k] != null && !islands[k].Unlocked) return islands[k];
                return null;
            }
        }

        public bool TryUnlock(Island i)
        {
            if (!CanUnlock(i) || _wallet == null) return false;
            if (!_wallet.TrySpendCash(new BigDouble(i.Def.UnlockCost))) return false;
            i.Unlock();
            if (_data != null && !_data.unlockedIslands.Contains(i.Def.name)) _data.unlockedIslands.Add(i.Def.name);
            return true;
        }

        /// <summary>Buy one level on the given track of an island (independent per island — writes only that
        /// island's save keys, so upgrading Coal never affects Iron).</summary>
        public bool TryUpgradeTrack(Island i, int track)
        {
            if (i == null || i.Def == null || !i.Unlocked || _wallet == null) return false;
            if (track < 0 || track >= i.TrackCount || i.TrackLevel(track) >= i.MaxLevelPerTrack) return false;
            if (!_wallet.TrySpendCash(i.TrackCost(track))) return false;
            i.TryUpgradeTrackLocal(track);
            SaveTrack(i, track);
            return true;
        }

        private void SaveTrack(Island i, int track)
        {
            if (_data == null) return;
            string id = i.Def.name + "#" + track;
            int level = i.TrackLevel(track);
            for (int j = 0; j < _data.islandLevels.Count; j++)
                if (_data.islandLevels[j].id == id) { _data.islandLevels[j].level = level; return; }
            _data.islandLevels.Add(new StationLevel { id = id, level = level });
        }

        /// <summary>Show only the given island's art (hide every other island — used when entering one).</summary>
        public void ShowOnly(Island only)
        {
            if (islands == null) return;
            for (int k = 0; k < islands.Length; k++) if (islands[k] != null) islands[k].SetVisible(islands[k] == only);
        }

        /// <summary>Show every island's art (back on the world map).</summary>
        public void ShowAll()
        {
            if (islands == null) return;
            for (int k = 0; k < islands.Length; k++) if (islands[k] != null) islands[k].SetVisible(true);
        }
    }
}
