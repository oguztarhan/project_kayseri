...
namespace Game.Gameplay
{
    ...
    public sealed class IslandOperation : MonoBehaviour
    {
        ...
        [SerializeField] private float loadSeconds = 1.2f;   // time spent hidden inside the mountain, loading ore
        ...
        private bool _insideMountain;  // hidden at the mine while loading
        private float _dist;
        ...
        {
            int lv = _island != null ? _island.TotalLevel : 0;
            return Mathf.Clamp(baseWagons + lv / 2, 1, maxWagons);   // +1 wagon every 2 upgrade levels
        }

        private void Update()
        {
            bool on = (alwaysRun || (_island != null && _island.Unlocked)) && (_island == null || _island.Visible);
            bool show = on && !_insideMountain;     // hidden while it's inside the mountain loading
            if (_train != null && _train.gameObject.activeSelf != show) _train.gameObject.SetActive(show);
            if (!on) return;
            ...
            if (_pause > 0f)
            {
                ...
                if (_pause <= 0f && _insideMountain) { _insideMountain = false; if (_train != null) _train.gameObject.SetActive(true); }  // emerge, full
                return;
            }

            float speed = baseSpeed * (1f + 0.25f * _island.TotalLevel);
            ...
            {
                _dist = 0f;
                _loaded = !_loaded;                 // just delivered (empty) or just reached the mine (about to load)
                SetOre(_loaded);                    // ore visible only while carrying to the depot
                Vector3 tmp = _from; _from = _to; _to = tmp;
                if (_loaded) { _pause = loadSeconds; _insideMountain = true; if (_train != null) _train.gameObject.SetActive(false); }  // drive into the mountain to load
                else _pause = endPause;             // brief stop at the depot
            }
        }
